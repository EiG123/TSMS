...
function useAVAChart() {
  ...
  const fetchData = async (params: {
    ...
    try {
      ...
      if (err.name !== "AbortError") error.value = err.message ?? "Unknown error";
    } finally {
      loading.value = false;
    }
  };
...
const aggregateChartData = computed(() => {
  const grouped: Record<string, { total: number; count: number; incidents: any[] }> = {};
  for (const row of chartData.value) {
    if (!grouped[row.date]) grouped[row.date] = { total: 0, count: 0, incidents: [] };
    grouped[row.date].total += Number(row.availability);
    grouped[row.date].count += 1;
    if (row.incidents?.length) grouped[row.date].incidents.push(...row.incidents);
  }
...
const totalPages = computed(() => Math.ceil(incidentData.value.length / pageSize));
...
const pageNumbers = computed(() => {
  ...
  for (let p = 1; p <= totalPages.value; p++) {
    if (p === 1 || p === totalPages.value || Math.abs(p - currentPage.value) <= 1) {
      pages.push(p);
...
watch(incidentData, () => { currentPage.value = 1; });
...
function selectAll() { selectedSites.value = [...ALL_site]; }
function clearAll() { selectedSites.value = []; }
...
function getAvailabilityBadgeClass(value: number): string {
  ...
  if (value < 99.5) return "bg-orange-100 text-orange-700 border border-orange-200";
  ...
  return {
    ...
    data: {
      ...
      datasets: [{
        ...
        pointBackgroundColor: aggregateChartData.value.map((d) =>
          d.date === selectedDate.value ? "#6366f1" : getAvailabilityColor(d.availability)
        ),
    ...
    options: {
      ...
      scales: {
        y: {
          min: yMin, max: yMax,
          ticks: { callback: (v) => `${v}%`, font: { family: "monospace" }, maxTicksLimit: 6 },
      ...
      plugins: {
        tooltip: {
          ...
          callbacks: {
            afterBody: (ctx) => {
              const incidents = aggregateChartData.value[ctx[0].dataIndex].incidents ?? [];
...
const exportCSV = () => {
  ...
  const rows = incidentData.value.map((r) => [r.date, r.subject, r.start, r.end]);
  ...
  <div class="min-h-screen bg-slate-50 font-mono p-6">
    <div class="max-w-5xl mx-auto space-y-6">
      ...
      <div class="flex items-baseline justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Availability Dashboard</h1>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
        <div class="flex flex-wrap gap-3 items-end">
          ...
          <div class="flex flex-col gap-1 relative">
            <label class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Site Code</label>
            <button
              type="button"
              ...
              :class="selectedSites.length ? 'text-slate-700' : 'text-slate-400'"
            >
              <span>{{ siteLabel }}</span>
              <span class="text-slate-400 ml-2">{{ dropdownOpen ? "▲" : "▼" }}</span>
            </button>
            ...
            >
              <!-- Actions row -->
              <div class="flex gap-2 px-3 py-2 border-b border-slate-100 shrink-0">
                <button @click="selectAll" class="text-xs text-indigo-600 hover:underline">All</button>
                <span class="text-slate-300">|</span>
                <button @click="clearAll" class="text-xs text-slate-400 hover:underline">Clear</button>
                <span class="ml-auto text-xs text-slate-400">{{ selectedSites.length }}/{{ ALL_site.length }}</span>
              </div>
              ...
              <div class="overflow-y-auto">
                ...
                <p v-if="filteredSites.length === 0" class="text-xs text-slate-400 px-3 py-2">No results</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-1">
            <label class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Start</label>
            <input
              type="date"
              v-model="startDate"
          ...
          <!-- End Date -->
          <div class="flex flex-col gap-1">
            <label class="text-xs text-slate-500 font-semibold uppercase tracking-wide">End</label>
            <input
              type="date"
              v-model="endDate"
              class="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
        ...
        <div v-if="selectedSites.length" class="flex items-center gap-2">
          <span class="text-xs bg-indigo-50 text-indigo-700 border border-indigo-200 px-2.5 py-1 rounded-full">
            {{ siteLabel }}
          </span>
          <button
            @click="clearAll"
      ...
      >
        ...
        <p class="text-slate-400 text-sm mt-1">Enter a site code and date range, then click Load</p>
      </div>
      ...
      <template v-else-if="chartData.length">
        ...
        <!-- Stats -->
        <div class="grid grid-cols-3 gap-4">
          <div v-for="(val, key) in stats" :key="key" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <p class="text-xs text-slate-400 uppercase tracking-widest font-semibold mb-1">{{ key }}</p>
            <p
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <p class="text-xs text-slate-400 uppercase tracking-widest font-semibold mb-4">
        ...
        >
          ...
          <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <div class="flex items-center gap-3">
              ...
              <span class="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">
                {{ incidentData.length }} record{{ incidentData.length > 1 ? "s" : "" }}
              </span>
              <span
                v-if="selectedDate"
              ...
              >
                {{ selectedDate }}
                <button @click="clearIncidentFilter" class="hover:text-indigo-800 font-bold">×</button>
              </span>
            </div>
            <button
              @click="exportCSV"
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="text-xs uppercase tracking-widest text-slate-400 bg-slate-50">
              ...
              <tbody>
                <tr
                  ...
                  :class="[
                    ...
                    hoveredRow === row.date ? 'bg-indigo-50' : 'hover:bg-slate-50',
                  ]"
                >
                  <td class="px-5 py-3 text-slate-700">{{ row.site || "Unknown" }}</td>
                  ...
                  <td class="px-5 py-3 text-slate-500 text-xs">{{ formatToReadable(row.fault_datetime) }}</td>
                  <td class="px-5 py-3 text-slate-500 text-xs">{{ formatToReadable(row.restoration_datetime) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          >
            <span>
              Showing {{ (currentPage - 1) * pageSize + 1 }}–{{ Math.min(currentPage * pageSize, incidentData.length) }}
