...
const navigations = [
  { name: "Summary", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z", action: () => alert("Go to SummaryPage") },
  { name: "Site Data", icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4", action: () => alert("Go to SiteDataPage") },
  { name: "Cabinets", icon: "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10", action: () => alert("Go to CabinetsPage") },
  { name: "Problems", icon: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z", action: () => alert("Go to ProblemsPage") },
  { name: "Audit", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4", action: () => alert("Go to AuditPage") }
];
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
    ...
    <div v-else-if="pMsiteData" class="max-w-7xl mx-auto space-y-6 animate-slide-up">
          ...
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-start gap-4">
              <div :class="[
                ...
                !isCheckedIn ? 'bg-gradient-to-br from-blue-500 to-blue-600 shadow-blue-500/30' :
                checkOutTime ? 'bg-gradient-to-br from-purple-500 to-purple-600 shadow-purple-500/30' :
                'bg-gradient-to-br from-green-500 to-green-600 shadow-green-500/30 animate-pulse'
              ]">
                <svg v-if="!isCheckedIn" class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
                </svg>
                <svg v-else-if="!checkOutTime" class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                <h3 class="text-xl font-semibold text-slate-200 mb-2">
                  {{ !isCheckedIn ? 'Ready to Start' : checkOutTime ? 'Work Completed' : 'In Progress' }}
                </h3>
                <p class="text-slate-400 text-sm mb-3">
                  {{ !isCheckedIn ? 'Click check in to start recording PM results' : 
                     checkOutTime ? 'All times have been recorded successfully' : 
                     'Currently working - ready to check out' }}
                </p>
                
                <div class="space-y-1.5">
                  <div v-if="checkInTime" class="flex items-center gap-2 text-sm">
                  ...
                  <div v-if="checkOutTime" class="flex items-center gap-2 text-sm">
            ...
            >
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path v-if="!isCheckedIn" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
                <path v-else-if="!checkOutTime" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span class="text-lg">
                {{ !isCheckedIn ? 'Check In' : checkOutTime ? 'Checked Out' : 'Check Out' }}
              </span>
            </button>
          </div>

          <!-- Status Bar -->
          <div v-if="isCheckedIn && !checkOutTime" class="mt-6 pt-6 border-t border-slate-700/50">
            <div class="flex items-center gap-3 px-4 py-3 bg-green-500/10 border border-green-500/30 rounded-xl">
              <div class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
              <span class="text-green-300 text-sm font-medium">Working - PM recording is active</span>
            </div>
          </div>

          <div v-if="checkOutTime" class="mt-6 pt-6 border-t border-slate-700/50">
            <div class="flex items-center gap-3 px-4 py-3 bg-purple-500/10 border border-purple-500/30 rounded-xl">
              <svg class="w-5 h-5 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span class="text-purple-300 text-sm font-medium">Work completed - All times recorded successfully</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="lg:col-span-2 bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-lg p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-5">
              ...
              <div class="group">
                <label class="block text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">PlanWork</label>
                <div class="bg-slate-900/40 border border-slate-700/50 rounded-lg px-4 py-3">
                  <p class="text-slate-200 font-medium">{{ pMsiteData.planwork || 'N/A' }}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <!-- Modules Section -->
      <div class="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-lg overflow-hidden">
        <div @click="hide" class="flex items-center justify-between px-8 py-6 cursor-pointer hover:bg-slate-800/60 transition-all duration-200 border-b border-slate-700/50">
          ...
          <button class="flex items-center gap-2 text-blue-400 hover:text-blue-300 font-medium text-sm transition-colors">
            ...
            <svg class="w-5 h-5 transition-transform duration-300" :class="{ 'rotate-180': !showModules }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="showModules" class="px-8 py-6 space-y-4">
          <!-- Mowing Module -->
          <div v-if="pMsiteData.mowing?.length" class="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl p-5 hover:from-green-500/15 hover:to-emerald-500/15 transition-all duration-200">
            <div class="flex items-center justify-between">
          ...
          <!-- Solar Cell Module -->
          <div v-if="pMsiteData.solar_cell?.length" class="bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/30 rounded-xl p-5 hover:from-yellow-500/15 hover:to-orange-500/15 transition-all duration-200">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-4">
          ...
          <div v-if="!pMsiteData.mowing?.length && !pMsiteData.solar_cell?.length" class="text-center py-8">
