...
const groupedTitleChild = computed(() => {
  ...
  return Object.fromEntries(
    Object.entries(groups).filter(([, items]) =>
      items.some((item) => item.status === "active" || item.img_status === "active")
    )
  );
});
...
<template>
  <div class="min-h-screen dark:bg-slate-900 p-4 md:p-8">
    ...
    <div v-else class="max-w-7xl mx-auto space-y-6 animate-slide-up">
      ...
      <!-- Header Section -->
      <div class="dark:bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-2xl overflow-hidden">
        <div class="relative">
          <div class="absolute inset-0 dark:bg-gradient-to-r from-blue-500/10 to-purple-500/10"></div>
        ...
        >
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg shadow-blue-500/30">
                  <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                  </svg>
                </div>
                <div>
                  <h3 class="text-xl font-semibold dark:text-slate-200">{{ groupName }}</h3>
                  <p class="text-sm text-slate-400 mt-0.5">
                    {{ items.length }} {{ items.length === 1 ? "item" : "items" }}
                  </p>
                </div>
              </div>
              <div class="px-3 py-1 bg-blue-500/20 text-blue-300 rounded-lg text-sm font-semibold border border-blue-500/30">
                {{ items.length }} Total
              </div>
            </div>
          </div>
          ...
          <div class="p-6 space-y-4">
            ...
            >
              ...
              <!-- Images Grid -->
              <div
                v-if="item.pm_images?.length > 0 && item.img_status === 'active'"
                class="p-6"
              >
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  >
                    <div class="aspect-square bg-gray-300 dark:bg-slate-900 overflow-hidden">
                      <img
                        v-if="img.file_path"
                        :src="img.file_path.startsWith('blob:') ? img.file_path : BASE_URL + img.file_path"
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div class="absolute bottom-0 left-0 right-0 p-3">
                        <p v-if="img.description" class="text-xs text-slate-200 font-medium">
                          {{ img.description }}
                        </p>
                        <p v-else class="text-xs text-slate-400">Image {{ imgIndex + 1 }}</p>
                      </div>
                    </div>

                    <!-- Image Number Badge -->
                    <div class="absolute top-2 right-2 w-8 h-8 rounded-lg bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 flex items-center justify-center">
                      <span class="text-xs font-bold text-slate-300">{{ imgIndex + 1 }}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <!-- Enter Data Button -->
              <div
                v-if="item.status === 'active' || item.img_status === 'active'"
                class="px-6 pb-6"
              >
                <button
      ...
      >
        <div class="text-center">
          ...
          <h3 class="text-xl font-semibold text-slate-300 mb-2">No Checklist Items</h3>
