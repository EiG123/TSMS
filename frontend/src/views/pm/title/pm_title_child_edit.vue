...
onMounted(async () => {
  try {
    const res_title_child_value = await pmTitleManage.getTitleChildById(title_child_id.value);

    const data = res_title_child_value.data.result;

    // ข้อมูล Basic จาก index 0
    title_child_name.value = data[0].title_child_name || "";
    description.value = data[0].description || "";
    status.value = data[0].status || "inactive";
    rank.value = data[0].rank || 0;

    // Values Section - loop จาก data array โดยตรง
    value_status.value = data[0].status || "inactive";

    if (data.length > 0) {
      value_number.value = data.length;
      await nextTick();
      // ✅ loop data array แทน data.values
      values.value = data.map((v: any) => ({
        name: v.name || "",
        input_type: v.input_type || "",
        dropdown_head_id: v.dropdown_head_id || null,
        dropdown_member_id: v.dropdown_member_id || null,
      }));
    }
