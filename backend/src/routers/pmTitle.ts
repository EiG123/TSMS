import { Hono } from "hono";
import { pmTitleService } from "../services/PmTitle.service.js";
import pool from "../services/db.js";

const pmTitleRouter = new Hono();

pmTitleRouter.post("/AddPmTitle", async (c) => {
    try {
        const body = await c.req.json();
        const {
            pm_name,
            pm_description,
            pm_key,
            pm_type,
            pm_status,
            pm_rank
        } = body;

        const result = await pmTitleService.InsertTitle(
            pm_name,
            pm_description,
            pm_key,
            pm_type,
            pm_status,
            pm_rank,
            pool
        );

        if (result.success) {
            // ⭐ ต้อง return pm_id กลับไปด้วย
            return c.json({
                success: true,
                message: "บันทึกข้อมูล PM Title สำเร็จ",
            });
        } else {
            return c.json(result, 401);
        }

    } catch (error) {
        console.error('PM Title error Router:', error);
        return c.json({
            success: false,
            message: "เกิดข้อผิดพลาดในการบันทึกข้อมูล",
        }, 500);
    }
});

pmTitleRouter.post("/EditpmTitle", async (c) => {
    try {
        const body = await c.req.json();
        const result = await pmTitleService.EditpmTitle(
            body,
            pool
        );

        if (result.success) {
            // ⭐ ต้อง return pm_id กลับไปด้วย
            return c.json({
                success: true,
                message: "บันทึกข้อมูล PM Title สำเร็จ",
            });
        } else {
            return c.json(result, 401);
        }

    } catch (error) {
        console.error('PM Title error Router:', error);
        return c.json({
            success: false,
            message: "เกิดข้อผิดพลาดในการบันทึกข้อมูล",
        }, 500);
    }
});

pmTitleRouter.post("/getAllPmTitle", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.getAllPmTitle(body.data, pool);
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/getPmTitleById", async (c) => {
    const body = await c.req.json();
    const { id } = body;
    const data = await pmTitleService.getPmTitleById(pool, id);
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/getAllPmTitleChild", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.getAllPmTitleChild(pool, body);
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/AddPmTitleChild", async (c) => {
    const body = await c.req.json();
    console.log(body);
    const data = await pmTitleService.InsertTitleChild(
        body.data,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/deleteTitleChildById", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.deleteTitleChildById(
        body,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/deleteTitleById", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.deleteTitleById(
        body,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/getTitleByType", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.getTitleByType(
        body,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});


pmTitleRouter.post("/getTitleChildByTitle", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.getTitleChildByTitle(
        body,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/getTitleChildDataByTitle", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.getTitleChildDataByTitle(
        body,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});


pmTitleRouter.post("/getTitleChildData", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.getTitleChildData(
        body.data,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});


pmTitleRouter.post("/EditpmTitleChild", async (c) => {
    const body = await c.req.json();
    console.log(body.data);
    const data = await pmTitleService.EditpmTitleChild(
        body.data,
        pool
    );
    if (data.success) {
        return c.json({
            data: data,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});

pmTitleRouter.post("/getAllTitleInfo", async (c) => {
    const body = await c.req.json();
    const data = await pmTitleService.getAllTitleInfo(
        body.data,
        pool
    );
    if (data.success) {
        return c.json({
            data: data.result,
            success: true
        });
    } else {
        return c.json({
            success: false
        });
    }
});


export default pmTitleRouter;
